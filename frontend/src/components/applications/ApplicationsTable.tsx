"use client";

import { useState } from "react";
import type { ApplicationListItem, ApplicationSortBy, ApplicationSortDir, ApplicationStatus } from "@/types/api";
import { applicationsApi } from "@/lib/api/applications";
import { formatTableDate } from "@/lib/applications/dates";
import { ApiError } from "@/lib/api/client";
import { cn } from "@/lib/utils";
import { STATUS_OPTIONS, statusLabel, jobTypeLabel, workModeLabel } from "@/lib/applications/presentation";
import { APPLICATION_COLUMN_DEFS, type ApplicationColumnId } from "@/lib/applications/tableColumns";
import { PILL_BASE_CLASS, getStatusPillTokens } from "@/lib/applications/pills";
import { getFitBadgeClass } from "@/lib/fit/presentation";
import { Button } from "@/components/ui/button";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { Alert } from "../ui/alert";
import { ChevronDown, ChevronUp, Star, X } from "lucide-react";


// ─── SortableHeader ───────────────────────────────────────────────────────────

// Helper to display a sortable column header in the table.
function SortableHeader({
  label,
  col,
  sortBy,
  sortDir,
  isDefaultSort,
  onSort,
  className,
}: {
  label:         string;
  col:           ApplicationSortBy;
  sortBy:        ApplicationSortBy;
  sortDir:       ApplicationSortDir;
  isDefaultSort: boolean;
  onSort:        (col: ApplicationSortBy) => void;
  className?:    string;
}) {
  const isActive  = sortBy === col;
  const showArrow = isActive && !isDefaultSort;

  return (
    <th
      className={cn("p-3 select-none", className)}
      aria-sort={isActive ? (sortDir === "asc" ? "ascending" : "descending") : "none"}
    >
      <button
        type="button"
        onClick={() => onSort(col)}
        className={cn(
          "w-full px-3 py-3 inline-flex items-center justify-start gap-1 text-left",
          "cursor-pointer select-none",
          "focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-sm"
        )}
      >
        <span className="whitespace-nowrap">{label}</span>
        {showArrow ? (
          sortDir === "asc"
            ? <ChevronUp   className="h-3 w-3 text-muted-foreground" />
            : <ChevronDown className="h-3 w-3 text-muted-foreground" />
        ) : null}
      </button>
    </th>
  );
}


// ─── StatusPillButton ─────────────────────────────────────────────────────────

/**
 * Renders the colored status pill as a clickable popover trigger.
 * Clicking opens a dropdown of all status options.
 * Selecting a new status calls onStatusChange and closes the popover.
 * Clicking elsewhere on screen closes the popover without any change.
 */
function StatusPillButton({
  applicationId,
  status,
  disabled,
  onStatusChange,
}: {
  applicationId:  string;
  status:         ApplicationStatus;
  disabled:       boolean;
  onStatusChange: (id: string, status: ApplicationStatus) => Promise<void>;
}) {
  const [open, setOpen] = useState(false);
  const { wrap, dot } = getStatusPillTokens(status);

  async function handleSelect(newStatus: ApplicationStatus) {
    // No-op if selecting the current status — just close
    if (newStatus === status) {
      setOpen(false);
      return;
    }
    setOpen(false);
    await onStatusChange(applicationId, newStatus);
  }

  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        {/* stopPropagation prevents the row click (drawer open) from firing */}
        <button
          type="button"
          disabled={disabled}
          aria-label={`Change status from ${statusLabel(status)}`}
          onClick={(e) => {
            e.stopPropagation();
            setOpen((prev) => !prev);
          }}
          className={cn(
            PILL_BASE_CLASS,
            wrap,
            "cursor-pointer transition-opacity hover:opacity-80",
            "focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1",
            disabled && "pointer-events-none opacity-50"
          )}
        >
          <span className={cn("w-1.5 h-1.5 rounded-full", dot)} />
          {statusLabel(status)}
          <ChevronDown className="h-3 w-3 ml-0.5 opacity-60" />
        </button>
      </PopoverTrigger>

      {/* Status option list */}
      <PopoverContent
        className="p-1 w-44"
        // Prevent the popover click from propagating to the row
        onClick={(e) => e.stopPropagation()}
      >
        {STATUS_OPTIONS.map((option) => {
          const { wrap: optWrap, dot: optDot } = getStatusPillTokens(option.value);
          const isCurrent = option.value === status;

          return (
            <div
              key={option.value}
              role="option"
              aria-selected={isCurrent}
              onClick={() => handleSelect(option.value)}
              className={cn(
                "flex items-center gap-2 rounded px-2 py-1.5 text-sm cursor-pointer transition-colors",
                "hover:bg-accent",
                isCurrent && "bg-muted font-medium"
              )}
            >
              {/* Mini colored dot matching the pill style */}
              <span className={cn("w-2 h-2 rounded-full shrink-0", optDot)} />
              <span className={cn(
                // Apply the text color from the pill tokens for visual consistency
                optWrap.split(" ").find((c) => c.startsWith("text-"))
              )}>
                {option.label}
              </span>
              {isCurrent && (
                <span className="ml-auto text-xs text-muted-foreground">✓</span>
              )}
            </div>
          );
        })}
      </PopoverContent>
    </Popover>
  );
}


// ─── ApplicationsTable ────────────────────────────────────────────────────────

/**
 * ApplicationsTable: read-only list with sortable columns and row actions.
 *
 * Status pill is clickable — opens an inline popover to change the application
 * status without opening the details drawer.
 */
export function ApplicationsTable({
  items,
  sortBy,
  sortDir,
  isDefaultSort,
  onSort,
  onChanged,
  visibleColumns,
  onRowClick,
}: {
  items:          ApplicationListItem[];
  sortBy:         ApplicationSortBy;
  sortDir:        ApplicationSortDir;
  isDefaultSort:  boolean;
  onSort:         (nextSortBy: ApplicationSortBy) => void;
  onChanged:      () => void;
  visibleColumns: ApplicationColumnId[];
  onRowClick?:    (application: ApplicationListItem) => void;
}) {
  const [rowError, setRowError] = useState<string | null>(null);

  // busyId: the application currently being updated/deleted (disables its row)
  const [busyId, setBusyId] = useState<string | null>(null);

  // Get definitions for the currently visible columns
  const visibleColumnsDefs = APPLICATION_COLUMN_DEFS.filter((c) =>
    visibleColumns.includes(c.id)
  );

  // ── Delete ────────────────────────────────────────────────────────────────

  async function handleDelete(id: string) {
    const ok = window.confirm("Delete this application?");
    if (!ok) return;

    setRowError(null);
    setBusyId(id);

    try {
      await applicationsApi.remove(id);
      onChanged();
    } catch (err) {
      if (err instanceof ApiError) setRowError(err.message);
      else setRowError("Failed to delete application.");
    } finally {
      setBusyId(null);
    }
  }

  // ── Inline status change ──────────────────────────────────────────────────

  async function handleStatusChange(id: string, newStatus: ApplicationStatus) {
    setRowError(null);
    setBusyId(id);

    try {
      await applicationsApi.updateStatus(id, newStatus);
      onChanged(); // trigger parent refetch so table row reflects new status
    } catch (err) {
      if (err instanceof ApiError) setRowError(err.message);
      else setRowError("Failed to update status.");
    } finally {
      setBusyId(null);
    }
  }


  // ── Render ────────────────────────────────────────────────────────────────

  return (
    <div className="space-y-2">

      {/* Row-level error banner */}
      {rowError ? (
        <div className="relative">
          <Alert variant="destructive" className="py-2 pr-10">
            {rowError}
          </Alert>
          <button
            type="button"
            onClick={() => setRowError(null)}
            className="absolute right-2 top-2 rounded-md px-2 py-1 opacity-70 hover:bg-black/5 hover:opacity-100"
            aria-label="Dismiss message"
          >
            ×
          </button>
        </div>
      ) : null}

      <div className="overflow-x-auto rounded-md border">
        <table className="min-w-[1100px] w-full text-sm">

          {/* Table header */}
          <thead className="bg-muted/50">
            <tr className="text-center">
              {visibleColumnsDefs.map((col) => {
                switch (col.id) {
                  case "favorite":
                    return (
                      <th key={col.id} className="p-3 w-[60px] text-center" title="Starred" />
                    );

                  case "salaryText":
                  case "actions":
                    return <th key={col.id} className="p-3">{col.label}</th>;

                  case "company":
                  case "position":
                  case "location":
                  case "jobType":
                  case "workMode":
                  case "status":
                  case "fitScore":
                  case "dateApplied":
                  case "updatedAt":
                  case "createdAt":
                    return (
                      <SortableHeader
                        key={col.id}
                        label={col.label}
                        col={col.id as ApplicationSortBy}
                        sortBy={sortBy}
                        sortDir={sortDir}
                        isDefaultSort={isDefaultSort}
                        onSort={onSort}
                      />
                    );

                  default:
                    return null;
                }
              })}
            </tr>
          </thead>

          {/* Table body */}
          <tbody>
            {items.length === 0 ? (
              <tr>
                <td
                  className="p-8 text-center text-muted-foreground"
                  colSpan={visibleColumnsDefs.length}
                >
                  <div className="space-y-1">
                    <div className="font-medium text-foreground">No applications to show</div>
                    <div className="text-sm text-muted-foreground">
                      Add an application above to get started.
                    </div>
                  </div>
                </td>
              </tr>
            ) : (
              items.map((application) => (
                <tr
                  key={application.id}
                  onClick={() => onRowClick?.(application)}
                  className={cn(
                    "border-t transition-colors hover:bg-muted/40 even:bg-muted/20",
                    onRowClick && "cursor-pointer",
                    busyId === application.id && "opacity-60"
                  )}
                >
                  {visibleColumnsDefs.map((col) => {
                    switch (col.id) {

                      case "favorite":
                        return (
                          <td key={col.id} className="p-3 w-[60px] text-center">
                            <Star
                              className={
                                application.isFavorite
                                  ? "h-4 w-4 fill-yellow-500 text-yellow-500"
                                  : "h-4 w-4 text-gray-400"
                              }
                            />
                          </td>
                        );

                      case "company":
                        return (
                          <td key={col.id} className="p-3 font-medium min-w-[150px]">
                            {application.company}
                          </td>
                        );

                      case "position":
                        return (
                          <td key={col.id} className="p-3 min-w-[150px]">
                            {application.position}
                          </td>
                        );

                      case "location":
                        return (
                          <td key={col.id} className="p-3 min-w-[150px]">
                            {application.location ?? "—"}
                          </td>
                        );

                      case "jobType":
                        return (
                          <td key={col.id} className="p-3 text-muted-foreground min-w-[140px]">
                            {jobTypeLabel(application.jobType)}
                          </td>
                        );

                      case "workMode":
                        return (
                          <td key={col.id} className="p-3 text-muted-foreground min-w-[150px]">
                            {workModeLabel(application.workMode)}
                          </td>
                        );

                      case "salaryText":
                        return (
                          <td key={col.id} className="p-3 text-muted-foreground min-w-[150px]">
                            {application.salaryText ?? "—"}
                          </td>
                        );

                      // Status cell: clickable pill opens inline status change popover.
                      // stopPropagation on the trigger prevents the row click (drawer) from firing.
                      case "status":
                        return (
                          <td key={col.id} className="p-3 text-center">
                            <StatusPillButton
                              applicationId={application.id}
                              status={application.status}
                              disabled={busyId === application.id}
                              onStatusChange={handleStatusChange}
                            />
                          </td>
                        );

                      case "fitScore": {
                        const score = application.fitScore;
                        return (
                          <td key={col.id} className="p-3 text-muted-foreground text-right">
                            {typeof score === "number" ? (
                              <span
                                className={cn(
                                  "inline-flex items-center rounded-md border px-2 py-0.5 text-sm font-medium tabular-nums",
                                  getFitBadgeClass(score)
                                )}
                                title={`${score}`}
                              >
                                {score}
                              </span>
                            ) : (
                              "—"
                            )}
                          </td>
                        );
                      }

                      case "dateApplied":
                        return (
                          <td key={col.id} className="p-3 text-muted-foreground text-center">
                            {formatTableDate(application.dateApplied)}
                          </td>
                        );

                      case "createdAt":
                        return (
                          <td key={col.id} className="p-3 text-muted-foreground text-center">
                            {formatTableDate(application.createdAt)}
                          </td>
                        );

                      case "updatedAt":
                        return (
                          <td key={col.id} className="p-3 text-muted-foreground text-center">
                            {formatTableDate(application.updatedAt)}
                          </td>
                        );

                      case "actions":
                        return (
                          <td key={col.id} className="p-3 text-right text-muted-foreground">
                            <Button
                              className="hover:text-destructive hover:bg-destructive/10"
                              variant="outline"
                              size="sm"
                              disabled={busyId === application.id}
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDelete(application.id);
                              }}
                            >
                              <X className="h-4 w-4" />
                            </Button>
                          </td>
                        );

                      default:
                        return null;
                    }
                  })}
                </tr>
              ))
            )}
          </tbody>
        </table>
      </div>
    </div>
  );
}
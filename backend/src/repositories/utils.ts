...
class BulkOperations {
   ...
   */
  static async createJobApplicationsWithRelations(
    userId: number,
    applications: Array<{
      company: string;
      position: string;
   ...
   */
  static async deleteUserWithAllData(userId: number) {
    return withTransaction(async (tx) => {
      ...
      const jobApps = await repositories.jobApplication.findMany(
        { userId },
        undefined,
        tx
      );
...
class SearchUtils {
   ...
   */
  static async advancedJobApplicationSearch(
    userId: number,
    criteria: {
      query?: string;
      status?: string;
    ...
    if (criteria.company) where.company = { contains: criteria.company, mode: 'insensitive' };
    if (criteria.dateFrom || criteria.dateTo) {
      where.dateApplied = {};
      if (criteria.dateFrom) where.dateApplied.gte = criteria.dateFrom;
      if (criteria.dateTo) where.dateApplied.lte = criteria.dateTo;
    }
    if (criteria.salaryMin || criteria.salaryMax) {
      where.salary = {};
      if (criteria.salaryMin) where.salary.gte = criteria.salaryMin;
      if (criteria.salaryMax) where.salary.lte = criteria.salaryMax;
    }
    ...
    return repositories.jobApplication.findManyWithPagination(
      where,
      {
        include: {
          documents: true,
...
class AnalyticsUtils {
   ...
   */
  static async getRecentActivity(userId: number, daysBack: number = 7) {
    ...
    const allActivity = [
      ...recentJobApps.map((item: any) => ({ type: 'job_application', data: item, createdAt: item.createdAt })),
      ...recentContacts.map((item: any) => ({ type: 'contact', data: item, createdAt: item.createdAt })),
      ...recentDocuments.map((item: any) => ({ type: 'document', data: item, createdAt: item.createdAt })),
   ...
   */
  static async getApplicationMetrics(userId: number, timeframe: 'week' | 'month' | 'quarter' | 'year' = 'month') {
    const now = new Date();
    ...
    if (applications.length > 0) {
      metrics.averageConnectionsPerApp = applications.reduce((sum: number, app: any) => sum + app.jobConnections.length, 0) / applications.length;
      metrics.averageDocumentsPerApp = applications.reduce((sum: number, app: any) => sum + app.documents.length, 0) / applications.length;
      
      const responsesReceived = applications.filter((app: any) => 
        ['interview_scheduled', 'offer_received', 'rejected'].includes(app.status)
      ).length;

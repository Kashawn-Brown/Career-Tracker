...
> {
   ...
   */
  async findByIdWithRelations(id: number, tx?: Prisma.TransactionClient): Promise<JobApplicationWithRelations | null> {
  ...
  async findStarredByUser(
    ...
    return this.findMany(
      { userId, isStarred: true }, // Filter by user and starred status
      {
        include: {
          documents: true, // Include all associated documents
  ...
  async findByStatus(
    ...
    const where: Prisma.JobApplicationWhereInput = { status };
    ...
    return this.findMany(
      where, // Apply status and optional user filters
      {
        include: {
          documents: true, // Include all associated documents
          jobConnections: true, // Include job connection details
   ...
   * @param tx - Optional transaction client to use for the operation
   */
  async toggleStarred(id: number, tx?: Prisma.TransactionClient): Promise<JobApplication> {
   ...
   */
  async getJobApplicationStats(userId: number, tx?: Prisma.TransactionClient): Promise<{
    total: number;
   ...
   */
  private processMonthlyStats(monthlyData: any[]): { month: string; count: number }[] {

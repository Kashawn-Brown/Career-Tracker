...
export async function listJobApplications(
  request: FastifyRequest<{ Querystring: ListJobApplicationsQuery }>,
  reply: FastifyReply
) {
  try {
    const {
    ...
    if (isStarred !== undefined) filters.isStarred = isStarred;
    if (hasFollowUp !== undefined) filters.hasFollowUp = hasFollowUp;
    if (salaryMin) filters.salaryMin = salaryMin;
    if (salaryMax) filters.salaryMax = salaryMax;
    if (compatibilityScoreMin) filters.compatibilityScoreMin = compatibilityScoreMin;

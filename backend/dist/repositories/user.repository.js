...
export class UserRepository extends BaseRepository {
     ...
     *   - Job connections/referrals
     */
    async findByIdWithJobApplications(id, tx) {
        return this.findById(id, {
            jobApplications: {
                orderBy: { dateApplied: 'desc' },
                include: {
     ...
     * - Contacts: ordered by creation date (newest first)
     */
    async findByIdWithContacts(id, tx) {
        return this.findById(id, {
            contacts: {
                orderBy: { createdAt: 'desc' },
            },
     ...
     */
    async isEmailTaken(email, excludeUserId, tx) {
     ...
     * @param tx - Optional transaction client to use for the operation
     */
    async updateProfile(id, data, tx) {
     ...
     */
    async getUserStats(userId, tx) {
        const client = tx || this.prisma;
        const [totalJobApplications, totalContacts, applicationsByStatus, recentApplications,] = await Promise.all([
     ...
     * @param tx - Optional transaction client to use for the operation
     */
    async searchUsers(query, options, tx) {
        const where = {
            OR: [

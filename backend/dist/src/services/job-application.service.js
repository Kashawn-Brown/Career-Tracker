...
export class JobApplicationService {
    ...
    async listJobApplications(filters) {
        const { page = 1, limit = 10, userId, status, company, position, dateFrom, dateTo, isStarred, hasFollowUp, salaryMin, salaryMax, compatibilityScoreMin, sortBy = 'dateApplied', sortOrder = 'desc' } = filters;
        // Build repository filters
        const repositoryFilters = {};
        if (userId)
            repositoryFilters.userId = userId;
        if (status)
            repositoryFilters.status = status;
        if (company)
            repositoryFilters.company = company;
        if (position)
            repositoryFilters.position = position;
        if (dateFrom)
            repositoryFilters.dateFrom = new Date(dateFrom);
        if (dateTo)
            repositoryFilters.dateTo = new Date(dateTo);
        if (isStarred !== undefined)
            repositoryFilters.isStarred = isStarred;
        if (hasFollowUp !== undefined)
            repositoryFilters.hasFollowUp = hasFollowUp;
        if (salaryMin)
            repositoryFilters.salaryMin = salaryMin;
        if (salaryMax)
            repositoryFilters.salaryMax = salaryMax;
        if (compatibilityScoreMin)
            repositoryFilters.compatibilityScoreMin = compatibilityScoreMin;
        // Build order by
        const orderBy = { [sortBy]: sortOrder };
        // Get paginated results
        return await repositories.jobApplication.findManyWithPagination(repositoryFilters, {
            include: {
                tags: true,
                documents: true,
                jobConnections: {
